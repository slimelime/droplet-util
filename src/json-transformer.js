...
const builtinFns = {
    ...
    toInteger: value => { const result = parseInt(value, 10); return _.isNaN(result) ? castingFunctionError({value, type: 'integer'}) : result; },
    toFloat: value => { const result = parseFloat(value, 10); return _.isNaN(result) ? castingFunctionError({value, type: 'float'}) : result; },
...
const transform = (template, {functions = {}, args = {}, throws = false, nullifyMissing = true} = {}, {builtins = builtinFns} = {}) => data => {
    ...
    return traverse(template).map(function (node) {
        ...
        if (node.startsWith('@')) {
            ...
            const fn = fns[fnName] || (throws ? () => { throw new Error(missingFunctionError({node})); } : colls.lazy(missingFunctionError({node})));
            ...
            const pipeline = fnNames.length === 0 ? colls.identity : colls.pipe(...colls.map(fnName => { return fns[fnName] || (throws ? () => { throw new Error(missingFunctionError({node})); } : colls.lazy(missingFunctionError({node}))); }, fnNames));
