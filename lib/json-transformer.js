...
const builtinFns = {
    ...
    matchesInList: (lst, source) => lst.filter(target => new RegExp(target, 'i').test(source)).length !== 0
};
...
const transform = (template, options = {}, { builtins = builtinFns } = {}) => data => {
    const { functions = {}, args = {}, throws = false, nullifyMissing = true, stopHere = true } = options;
    ...
    return traverse(template).map(function (node) {
        ...
        if (node.startsWith('@')) {
            ...
            that.update(pipeline(...argList), stopHere);
            ...
            that.update(value, stopHere); // if path is not found in data, the key would disappear upon JSON.stringify the result.
        }
    });
};
...
function allNodes(path = '$..*', data) {
    return jp.nodes(data, path).map(({ path, value }) => ({ path: jp.stringify(path), value }));
}
...
const filter = (template, { functions = {}, args = {}, throws = false, nullifyMissing = true } = {}, { builtins = builtinFns } = {}) => data => {
    ...
    const ruleNodes = [...allNodes('$..ANY', rules).map(entry => (0, _extends3.default)({}, entry, { operator: 'ANY' })), ...allNodes('$..ALL', rules).map(entry => (0, _extends3.default)({}, entry, { operator: 'ALL' }))].sort(sortBy('path', { asc: false })); // depth first

    // logger.log({ruleNodes});
    for (const _ref of ruleNodes) {
        const { operator, path, value } = _ref;
