...
const identity = x => x;
const identityAsync = x => _promise2.default.resolve(x);
const lazy = x => () => x;
...
const composeAsync = (...fns) => reduceAsync((fn1, fn2) => (() => {
    var _ref = (0, _asyncToGenerator3.default)(function* (...args) {
        return fn1((yield fn2(...args)));
    });
...
const GeneratorFunctionType = function* () {}.constructor;
const AsyncFunctionType = (0, _asyncToGenerator3.default)(function* () {}).constructor;

/** ----- FUTURE ----- **/

const SymbolIterator = _iterator3.default;
const SymbolAsyncIterator = _symbol2.default.asyncIterator;
...
const isEmptyValue = x => isNil(x) || !isNumber(x) && !isFunction(x) && (0, _keys2.default)(x).length === 0; // works for null, undefined, '', [], {}
...
function* entries(o, values = false, kv = true) {
    const entryKeys = (0, _keys2.default)(o);
    if (values) {
...
function iterator(o, { indexed = false, kv = false, metadata = lazy({}) } = {}) {
    ...
    if (isGenerator(o)) {
        ...
        iter = indexed ? toIterator((0, _getIterator3.default)(o), indexed) : (0, _getIterator3.default)(o);
    } else if (isObject(o)) {
 ...
 */
const reduceAsync = (() => {
    var _ref3 = (0, _asyncToGenerator3.default)(function* (reducingFn, initFn, enumerable) {
        let result;
        ...
        if (!initFn) {
            if (isAsync) {
                initFn = lazy((yield iter.next()).value);
            } else {
                const [initValue] = iter;
                initFn = lazy(initValue);
            }
        ...
        if (isAsync) {
            ...
            try {
                for (var _iterator = (0, _asyncIterator3.default)(iter), _step, _value; _step = yield _iterator.next(), _iteratorNormalCompletion = _step.done, _value = yield _step.value, !_iteratorNormalCompletion; _iteratorNormalCompletion = true) {
                    const value = _value;
                    // see: https://babeljs.io/docs/plugins/syntax-async-generators/
                    result = yield reducingFn((yield result), (yield value));
                }
            } catch (err) {
...
const appendAsync = (reducingFn, { factory = identity } = {}) => (() => {
    var _ref4 = (0, _asyncToGenerator3.default)(function* (acc, input) {
        return factory([...(yield acc), input]);
    });

    return function (_x4, _x5) {
        return _ref4.apply(this, arguments);
    };
})(); // `await acc` is just a precaution, reduceAsync() already await for previous result from the reducing function
...
const catAsync = (reducingFn, { factory = identity } = {}) => (() => {
    var _ref5 = (0, _asyncToGenerator3.default)(function* (acc, input) {
        return factory([...(yield acc), ...input]);
    });
...
// update:: fn -> acc -> x -> acc
const update = (reducingFn, { factory = identity } = {}) => (acc, [key, value]) => factory((0, _extends3.default)({}, acc, { [key]: value }));
const mapUpdate = (fn, iterable) => reduce(compose(mapTransformer(fn), update)(), () => ({}), iterable);
...
const map = (fn, iterable) => reduce(compose(mapTransformer(fn), append)(), () => [], iterable);

const mapAsyncTransformer = mappingFn => reducingFn => (() => {
    var _ref6 = (0, _asyncToGenerator3.default)(function* (acc, input) {
        return reducingFn(acc, (yield mappingFn(input)));
    });
...
const mapAsync = (() => {
    var _ref7 = (0, _asyncToGenerator3.default)(function* (fn, enumerable) {
        return reduceAsync((yield (yield composeAsync(mapAsyncTransformer(fn), appendAsync))()), function () {
            return [];
        }, enumerable);
    });

    return function mapAsync(_x10, _x11) {
        return _ref7.apply(this, arguments);
    };
})();

const filterTransformer = predicateFn => reducingFn => (acc, input) => predicateFn(input) ? reducingFn(acc, input) : acc;
const filter = (fn, iterable) => reduce(compose(filterTransformer(fn), append)(), () => [], iterable);

const filterAsyncTransformer = predicateFn => reducingFn => (() => {
    var _ref8 = (0, _asyncToGenerator3.default)(function* (acc, input) {
        return (yield predicateFn(input)) ? reducingFn(acc, input) : acc;
    });
...
const filterAsync = (() => {
    var _ref9 = (0, _asyncToGenerator3.default)(function* (fn, enumerable) {
        return reduceAsync((yield (yield composeAsync(filterAsyncTransformer(fn), appendAsync))()), function () {
            return [];
        }, enumerable);
    });

    return function filterAsync(_x14, _x15) {
